	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) {
		...
		if (user != null) {
			...
			if(user.getStatus() != null &&
					(user.getStatus().intValue() == UserConstant.STATUS_AUDIT || 
					user.getStatus().intValue() == UserConstant.STATUS_REJECT)) {
				throw new AuthenticationException("msg:用户未审核或审核不通过.");
			}
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (user != null) {
			...
			if(user.getOffice() != null && user.getOffice().getId() != null) {
				UserUtils.setOrg(user.getOffice().getId());
			}
			// 更新登录IP和时间
			getSystemService().updateUserLoginInfo(user);
			
			UserUtils.officeRefences();
			// 记录登录日志
