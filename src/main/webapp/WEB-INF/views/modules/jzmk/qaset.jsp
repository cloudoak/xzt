<%@ page contentType="text/html;charset=UTF-8" %>
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
<!DOCTYPE html>
<html>
<!-- Mirrored from www.zi-han.net/theme/hplus/form_validate.html by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 20 Jan 2016 14:19:15 GMT -->
<head>

    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <title>量表管理</title>
    <meta name="keywords" content="${fns:getConfig('keywords')} ${sysConfig.schoolName } ">
    <meta name="description" content="${fns:getConfig('description')} ${sysConfig.schoolName }">
    
    <link href="${hplusStatic }/favicon.ico" rel="shortcut icon">
	<link href="${hplusStatic }/css/bootstrap.min14ed.css?v=3.3.6" rel="stylesheet">
    <link href="${hplusStatic }/css/font-awesome.min93e3.css?v=4.4.0" rel="stylesheet">
    <link href="${hplusStatic }/css/animate.min.css" rel="stylesheet">
    <link href="${hplusStatic }/css/style.min862f.css?v=4.1.0" rel="stylesheet">
    <link href="${hplusStatic }/css/plugins/bootstrap-table/bootstrap-table.min.css" rel="stylesheet">
    <link href="${hplusStatic }/css/plugins/awesome-bootstrap-checkbox/awesome-bootstrap-checkbox.css" rel="stylesheet">
    <link href="${hplusStatic }/css/plugins/summernote/summernote.css" rel="stylesheet">
    <link href="${hplusStatic }/css/plugins/summernote/summernote-bs3.css" rel="stylesheet">
    <link href="${hplusStatic }/css/plugins/iCheck/custom.css" rel="stylesheet">
    
	<link href="${serverUrl }css/measure.css" type="text/css" rel="stylesheet" />

</head>

<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row">
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5><a class="text-navy" href="${ctx}/jzmk/scale/form?id=${sid}">量表添加</a></h5>
                        <div class="ibox-tools">
                        </div>
                    </div>
                    <div class="ibox-content">
                        <form class="form-horizontal m-t" id="questionForm" action="${ctx}/jzmk/gene/geneList" method="post">
                            <input type="hidden" name="sid" value="${sid }">
                            <input type="hidden" name="tid" value="${sid }">
                            <div class="form-group">
                                <label class="col-sm-3 control-label"></label>
                                <div class="col-sm-12">
			                    	<div class="liangbao">
							         	<a href="#" class="hover"><em class="em1 hover"></em>01基本信息</a>
							            <a href="#" class="hover"><em class="em2 hover"></em>02条目设置</a>
							            <a href="#"><em class="em3"></em>03因子设置</a>
							            <a href="#"><em class="em4"></em>04因子解释</a>
							            <a href="#"><em class="em5"></em>05总解释</a>
							         </div>
						        </div>
                            </div>
                            <div class="example">
                    		<div class="btn-group " id="exampleTableEventsToolbar" role="group">
                                <button type="button" class="btn btn-outline btn-default" onclick="addEntry(${sid});">
                                    <a class="J_menuItem">
                                    <i class="glyphicon glyphicon-plus text-navy" aria-hidden="false">添加条目</i>
                                   	</a>
                                </button>
                                <button id="btnSetAnswer" type="button" class="btn btn-outline btn-default">
                                    <i class="glyphicon glyphicon-edit text-navy" aria-hidden="false">设置答案</i>
                                </button>
                            </div>
                            <div class="form-group">
                            <label class="col-sm-3 control-label"></label>
                            <div class="col-sm-8">
                            <input type="text" id="isNoAnswer" style="opacity:0;border:none;outline:medium;" name="isNoAnswer" value="${sid }" />
                            <input type="hidden" id="inside" name="inside" value="${inside }" />
                            </div>
                            </div>
                            <table id="exampleTableToolbar" data-toggle="table" data-query-params="queryParams"  data-page-size="100" 
                             data-mobile-responsive="true" data-height="400" data-pagination="true" data-search="true">
                                <thead>
                                    <tr>
                                        <th data-field="" data-checkbox="true" aria-hidden="true"></th>
                                        <th data-field="id" data-visible="false">ID</th>
                                        <th data-field="numbers" data-visible="false">number</th>
                                        <th data-field="number">序号</th>
                                        <th data-field="content">条目</th>
                                        <c:if test="${inside == 1 }">
                                        <th data-field="parentId">父条目</th>
                                        </c:if>
                                        <th>上移</th>
                                        <th>下移</th>
                                        <th>操作</th>
                                    </tr>
                                </thead>
                                <c:forEach items="${page.list }" var="question" varStatus="status">
									<tr>
										<td data-field="id" data-checkbox="true" aria-hidden="true"></td>
										<td>${question.id }</td>
										<td>${question.number}</td>
										<td><a href="${ctx}/jzmk/question/form?id=${question.id}&sid=${question.sid}">${question.number}</a></td>
										<td class="${question.isSetAnswer==0?'text-danger':'' }">${question.content}</td>
										<c:if test="${inside == 1 }">
                                        <td>
                                        ${fn:length(fn:split(question.parentId, ','))}项:${question.parentId}
                                        </td>
                                        </c:if>
										<td>
											<a href="${ctx}/jzmk/question/update?op=up&id=${question.id}&sid=${question.sid}"><i class="glyphicon glyphicon-arrow-up text-navy"></i></a>
										</td>
										<td>
											<a href="${ctx}/jzmk/question/update?op=down&id=${question.id}&sid=${question.sid}"><i class="glyphicon glyphicon-arrow-down text-navy"></i></a>
										</td>
										<td>
											<a class="text-navy" href="${ctx}/jzmk/question/form?id=${question.id}&sid=${question.sid}">
												<i class="glyphicon glyphicon-edit" aria-hidden="true"></i>
											</a>&nbsp;
											<a href="javascript:delQuestion('${question.id}','${question.sid}');" class="text-danger">
												<i class="glyphicon glyphicon-trash" aria-hidden="true"></i>
											</a>
										</td>
									</tr>
								</c:forEach>
                            </table>
							<div class="form-group">
                                <div class="col-sm-8 col-sm-offset-3">
                                    <button class="btn btn-primary" type="button" onclick="gotoScaleFormPage(${sid})">上一步</button>&nbsp;
                                    <button class="btn btn-primary" type="submit">下一步</button>
                                </div>
                            </div>
                        </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <script src="${hplusStatic }/js/jquery.min.js?v=2.1.4"></script>
    <script src="${hplusStatic }/js/bootstrap.min.js?v=3.3.6"></script>
    <script src="${hplusStatic }/js/content.min.js?v=1.0.0"></script>
    <script src="${hplusStatic }/js/plugins/validate/jquery.validate.min.js"></script>
    <script src="${hplusStatic }/js/plugins/validate/messages_zh.min.js"></script>
    <script src="${hplusStatic }/js/demo/form-validate-demo.min.js"></script>
    <script src="${hplusStatic }/js/plugins/summernote/summernote.min.js"></script>
    <script src="${hplusStatic }/js/plugins/summernote/summernote-zh-CN.js"></script>
    <script src="${hplusStatic }/js/plugins/iCheck/icheck.min.js"></script>
    <script src="${hplusStatic }/js/plugins/bootstrap-table/bootstrap-table.min.js"></script>
    <script src="${hplusStatic }/js/plugins/bootstrap-table/bootstrap-table-mobile.min.js"></script>
    <script src="${hplusStatic }/js/plugins/bootstrap-table/locale/bootstrap-table-zh-CN.min.js"></script>
  	<script src="${hplusStatic }/js/demo/bootstrap-table-demo.min.js"></script> 
    <script src="${hplusStatic }/js/plugins/layer/layer.min.js" type="text/javascript"></script>
    <script>
	    function gotoScaleFormPage(id){
	    	document.location.href = "${ctx}/jzmk/scale/form?id=" + id;
	    }
	    function windowOpen(url, name, width, height){
	    	var top=parseInt((window.screen.height-height)/2,10),left=parseInt((window.screen.width-width)/2,10),
	    		options="location=no,menubar=no,toolbar=no,dependent=yes,minimizable=no,modal=yes,alwaysRaised=yes,"+
	    		"resizable=yes,scrollbars=yes,"+"width="+width+",height="+height+",top="+top+",left="+left;
	    	window.open(url ,name , options);
	    }
	    function addEntry(sid){
	    	location.href="${ctx}/jzmk/question/form?sid=" + sid;
	    }
    	//删除条目
	    function delQuestion(id,sid){
			layer.confirm('是否删除该条目？', {
			    btn: ['确认','取消'], //按钮
			    title:'删除确认',
			    shade: false //不显示遮罩
			}, function(){
				document.location.href="${ctx}/jzmk/question/delete?id="+id+"&sid="+sid;
			    layer.msg('删除成功！', {icon: 6});
			}, function(){
			    layer.msg('已取消', {icon: 1});
			});
		}
    	//设置答案
    	$("#btnSetAnswer").click(function(){
    		 var a = $.map($("#exampleTableToolbar").bootstrapTable('getSelections'), function (row) {
                 return row.id;
             });
    		 var c = $.map($("#exampleTableToolbar").bootstrapTable('getSelections'), function (row) {
                 return row.numbers;
             });
	   		if(a.length<=0){
	   			layer.msg('请选择你要设置答案的条目', {icon: 5});
	   		}else{
			var ids = JSON.stringify(a), numbers = JSON.stringify(c);
			document.location.href="${ctx}/jzmk/answer/form?ids="+ids+"&sid=${sid}&numbers="+numbers;
	   		} 
    	});
    	
    	$("#questionForm").validate({
    		errorElement: 'div',
    		errorClass: 'help-block',
    		focusInvalid: false,
    		rules: {
    			isNoAnswer: {
    				remote: {
    					url: "${ctx}/jzmk/question/checkAnswer",
                        type: "post",
                        dataType: "json",
                        data: {
                        	isNoAnswer: function () {
                               return $("#isNoAnswer").val();
                          	},
                          	inside: function () {
                          		return $("#inside").val();
                          	}
                        },
                        dataFilter: function (data) {
                        	return (data == 1);
                        }
    				}
	    		}
	    	},messages: {
	    		isNoAnswer: {
	    			remote: '条目下没有设置答案,必须要设置答案,请设置答案后再继续！'
	    		}
	    	},
	    	invalidHandler: function (event, validator) { //display error alert on form submit   
				$('.alert-danger', $('.login-form')).show();
			},
			highlight: function (e) {
				$(e).closest('.form-group').removeClass('has-info').addClass('has-error');
			},
			success: function (e) {
				$(e).closest('.form-group').removeClass('has-error').addClass('has-info');
				$(e).remove();
			},
			errorPlacement: function (error, element) {
				 error.appendTo(element.parent());  
			},
			submitHandler: function (form) {
				 form.submit();
			}
    	});

        $(function(){
        	$(".i-checks").iCheck({checkboxClass:"icheckbox_square-green",radioClass:"iradio_square-green"})
        });
    </script>
</body>
</html>